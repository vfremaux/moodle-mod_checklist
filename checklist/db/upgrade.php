...
function xmldb_checklist_upgrade($oldversion=0) {
    ...
    if ($result && $oldversion < 2011082001) {
        $table = new XMLDBTable('checklist');
        $field = new XMLDBField('lockteachermarks');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '4', XMLDB_UNSIGNED, null, null, null, null, '0');
        $result = $result && add_field($table, $field);
    }

    if ($result && $oldversion < 2012040600) {
        $table = new XMLDBTable('checklist');
        $field = new XMLDBField('usetimecounterpart');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '1', XMLDB_UNSIGNED, null, null, null, null, '0', 'lockteachermarks');
        $result = $result && add_field($table, $field);
    }

    if ($result && $oldversion < 2012040601) {
        $table = new XMLDBTable('checklist_item');
        $field = new XMLDBField('credittime');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, null, null, null, null, '0', 'duetime');
        $result = $result && add_field($table, $field);
    }
