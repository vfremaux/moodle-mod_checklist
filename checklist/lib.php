...
function checklist_delete_instance($id) {
    ...
    if ($items) {
        ...
        delete_records_select('checklist_comment', 'itemid IN ('.$items.')');
        delete_records('checklist_item', 'checklist', $checklist->id);
    }
...
function checklist_cron () {
	...
	if (!optional_param('forceall', false, PARAM_BOOL)){
    ...
    $sql = "
    	...
    	WHERE 
    		g.itemid = i.id AND 
    		g.timemodified >= $lastlogtime AND 
    		i.itemmodule IN ('quiz','assignment','forum') AND 
    		i.courseid IN ($courseids) 
    ";
