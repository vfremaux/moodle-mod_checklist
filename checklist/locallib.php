...
class checklist_class {
    ...
    function get_items() {
        ...
        $select = " 
        	checklist = {$this->checklist->id} AND 
        	userid = 0
        ";
        $this->items = get_records_select('checklist_item', $select, 'position');
        ...
        if ($this->userid && $this->canaddown()) {
            $select = "
            	...
            	userid = {$this->userid}
            ";
            $this->useritems = get_records_select('checklist_item', $select, 'position, id');
        } else {
        ...
        if ($this->userid) { // && ($this->canupdateown() || $this->canviewreports() )) {
            ...
            $sql = "
            	...
            	LEFT JOIN 
            		{$CFG->prefix}checklist_check c 
            	ON 
            		(i.id = c.item AND 
            		 c.userid = {$this->userid}) 
            	WHERE 
            		i.checklist = {$this->checklist->id}
            ";
    ...
    function update_items_from_course() {
        ...
        while ($section = next($sectionarr)) {
            ...
            if (!$sectionheading) {
            	if (checklist_course_is_page_formatted()){
            		$pagename = get_field('format_page', 'nametwo', 'id', $section);
	                $name = get_string('page', 'format_page').': '.$pagename;
	            } else {
            ...
            foreach($sectioncms as $cmid) {
                ...
                if (preg_match('/label$/', $modname) || preg_match('/pagemenu$/', $modname)) {
                    continue; // Ignore any labels
                }
                if (preg_match('/.*assignment$/', $modname) || preg_match('/.*quiz$/', $modname) || $modname == 'forum') {
                    $showscore = true;
                } else {
                    $showscore = false;
                }
        ...
        if (!$this->items) {
            ...
            foreach ($this->items as $item) {
                ...
                if (!$foundcomment && $editcomments) {
                    echo "&nbsp;<br />$commentstr <input type=\"text\" name=\"teachercomment[{$item->id}]\" size=\"70\" /><br/><br/>";
                }
    ...
    function view_report() {
        ...
        if ($editchecks) {
            ...
            echo '<input type="submit" name="submit" value="'.get_string('savechecksandeditoff', 'checklist').'" />';
        ...
        if ($showbars) {
            ...
            if ($editchecks) {
                echo '<center>
                <br/><input type="submit" name="submit" value="'.get_string('savechecks','checklist').'" />
                </center>
                ';
                echo '</form>';
            }
    ...
    function print_report_table($table, $editchecks) {
        ...
        $output .= ' cellpadding="5" cellspacing="1" class="generaltable boxaligncenter checklistreport" width="100%">';
                    ...
                    if ($heading) {
                        ...
                        if ($showteachermark) {
                            ...
                            if ($editchecks) {
                                ...
                                $img .= '<option value="'.CHECKLIST_TEACHERMARK_YES.'" '.$sely.'>'.get_string('validate', 'checklist').'</option>';
                                $img .= '<option value="'.CHECKLIST_TEACHERMARK_NO.'" '.$seln.'>'.get_string('unvalidate', 'checklist').'</option>';
                                $img .= '</select>';
                            }
                        }
                        if ($showstudentmark) {
    ...
    function view_own_report(){
        ...
        $sql = "
        	...
        	LEFT JOIN 
        		{$CFG->prefix}checklist_check c 
	        ON 
	        	(i.id = c.item AND c.userid = {$USER->id}) 
	        WHERE 
	        	i.checklist = {$this->checklist->id} AND 
	        	i.userid=0 
	        ORDER BY 
		...
		if ($checks){
	    	...
	    	$nochecksstr = get_string('nochecks', 'checklist');
	    	echo "<div class=\"advice\">$nochecksstr</div>";
	    }
        
        echo '<br/>';

        if ($this->checklist->autopopulate == CHECKLIST_AUTOPOPULATE_COURSE){
	        $completionliststr = get_string('coursecompletionboard', 'checklist');
	    } else {
	        $completionliststr = get_string('completionboard', 'checklist');
	    }
        echo "<div class=\"sideblock\"><div class=\"header\"><h2>$completionliststr</h2></div></div>";
    	echo "<table class=\"checklistreport\" cellspacing=\"4\"><tr valign=\"top\">";
        $studentimg = array(0 => '<img src="'.$CFG->wwwroot.'/pix/spacer.gif" alt="'.get_string('studentmarkno','checklist').'" />',
                            1 => '<img src="'.$CFG->wwwroot.'/pix/i/tick_amber_big.gif" alt="'.get_string('studentmarkyes','checklist').'" />');
		$i = 0;
    	foreach($checkstates as $cs){
			...
			if (!$heading){
				...
				if ($comments = get_records_select('checklist_comment', " userid = $USER->id AND itemid = $checkid ")){
					echo "<div class=\"comment\">";
					foreach($comments as $comment){
						$commenter = get_record('user', 'id', $comment->commentby, '', '', '', '', 'id,firstname,lastname');
						$commentername = get_string('reportedby', 'checklist', fullname($commenter));
			...
			if ($i % CHECKLIST_MAX_CHK_MODS_PER_ROW == 0 && $i > 0){
    ...
    function additem($displaytext, $userid=0, $indent=0, $position=false, $duetime=false, $moduleid=0, $optional=CHECKLIST_OPTIONAL_NO, $hidden=CHECKLIST_HIDDEN_NO) {
        ...
        $item->id = insert_record('checklist_item', addslashes_object($item));
...
function checklist_course_is_page_formatted(){
	global $COURSE;
